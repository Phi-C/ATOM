...
class MiniMaxM3SparseAttention(nn.Module):
    ...
    def __new__(cls, *args, **kwargs):
        if cls is MiniMaxM3SparseAttention:
            from atom.plugin.prepare import is_sglang

            if is_sglang():
                from atom.plugin.sglang.models.minimax_m3 import (
                    SGLangMiniMaxM3SparseAttention,
                )

                return SGLangMiniMaxM3SparseAttention(*args, **kwargs)
...
class MiniMaxM3DecoderLayer(nn.Module):
    ...
    def forward(
        ...
        if is_sglang():
            # Match fused_allreduce_gemma_rms_norm semantics while avoiding its
            # HIP fused residual path in the SGLang M3 bring-up.
            if get_tensor_model_parallel_world_size() > 1:
                hidden_states = tensor_model_parallel_all_reduce(hidden_states)
            hidden_states, residual = self.post_attention_layernorm.forward_native(
